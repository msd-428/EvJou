...
> **【2026-08-21 追記】以下は 2026-07-29 時点の記録で、数値と表示は現行と異なります。**
> **当時の観測記録なので数値は書き換えません**（`.agents/PROJECT_RULES.md` §4-2）。
> 現行の正は次のとおりです。
>
> - **`DAILY_LIMIT` の既定は 50**（`proxy/index.js:18`）。下記3項の「200回/日」は当時の値で、
>   `52e4ec5`「AI利用回数のUI非表示化と上限数の引き下げ（200→50）」で変更済み
> - **「本日のAI利用回数: N 回 / M」の表示は存在しません。** 同じ `52e4ec5` でUIから非表示化済み。
>   下記4項と、後述の動作確認ログに出てくるこの表示は、**現在は再現しません**
>
> 運用上の正は `docs/operations.md`、現在地は `.agents/STATE.md` を見てください。
