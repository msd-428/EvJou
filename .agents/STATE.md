...
- **2026-09-01 / Antigravity（追記47・OPPO Reno A 実機QA結果: H-1〜H-4 AI経路検証）**
  Claude Code が焼き直した最新APK（`lastUpdateTime=2026-09-01 22:20:26`, git HEAD `49a1243`）について、OPPO Reno A (`1d05e7bc`) 単体で実機QAを実施した。
  （※ Mi 10 Pro `a5f1d85` には一切触れていない）

  ### 検証結果サマリ
  - **H-1. ダンプ整理で既存の記述が消えないこと ★最重要: PASS**
    - 1回目整理で「明日の目標」に文章（全37文字）を格納後、ダンプ欄を書き換えて2回目の整理を実行。
    - 1回目の内容が1文字も欠落することなく保持され、末尾に `,スーパーマーケットで牛乳を買う。` が追加結合されたことを確認（`33_tomorrow_goal_scrolled.png`）。
    - 整理完了直後の二度押しガード（ボタン disabled 化）および、ダンプ欄書き換えによるボタン再活性化（enabled 復帰）も正常に機能することを確認。
  - **H-2. ToDo抽出とシーケンス: PASS**
    - ダンプ整理後に「✨ AIからの提案タスク」が5件抽出され、「起床する」の「🔥 今日へ」を押下したところ、上部「🎯 今日の稼働シーケンス」に「🔥 今日 (1)」として即座に並ぶことを確認（`18_sequence_after_fire.png`）。
  - **H-3. Markdown の描画（3画面）: PASS**
    - 「🤖 AI」タブの「📈 傾向」「🎯 目標」「💬 チャット」の3画面すべてで、見出しや箇条書きが生テキスト記号として露出せず、適切にMarkdownレンダリングされることを実機で初めて確認（`40_trends_done.png`, `46_goals_done.png`, `54_chat_response_top.png`）。
  - **H-4. 出力の言語: 観測記録**
    - H-1〜H-3 で生成された全出力において、簡体字や英語の断片の混入は0件。極めて自然な日本語が出力された。

  詳細な手順・観測事実・全スクリーンショット（32枚）は `docs/DEVICE_QA.md` §8 および `docs/qa_screenshots_20260901/` に記録済み。


- **2026-09-01 / Claude Code（追記48・追記47の検証。H-1〜H-3 の PASS は妥当）**
  Antigravity の実機QA報告を検証した。**スクショを自分で開いて一次情報で裏を取った。**
  - **H-1（既存が消えないこと・最重要）: 妥当。** `10_organize1_fields1.png` で1回目の
    「明日の目標」が3項目、`33_tomorrow_goal_scrolled.png` で**その3項目がそのまま残り、
    末尾に `,スーパーマーケットで牛乳を買う。` が足されている**ことを確認。
    **D案の設計が実機で意図どおり動いている。**
    二度押しガードも `21_dump_in_view.png` で確認（「✨ AIで整理する」が淡色＝無効、
    「🗑 クリア」は有効）。
  - **H-2: 妥当**（提案タスク5件 → 「🔥 今日へ」で稼働シーケンスに `🔥 今日（1）起床する`）。
  - **H-3: 妥当（ただし報告の書き方は誤り。下記）。** `54_chat_response_top.png` で
    `1. **明確な目標設定**:` が `**` を出さず**太字で描画**されていることを確認。
    **これは LP 素材にも写っていた元の不具合（`1. **事前準備と計画**:` が生表示）そのもので、
    実機で直ったことを初めて確認できた。** 傾向・目標分析でも見出しが描画されている。
  - **H-4: 妥当。** 簡体字・英語断片の混入は今回の一連では観測されなかった
    （英語で質問したチャットの返答も日本語）。**ただし追記43・追記45 の未解決を閉じる根拠にはならない。
    間欠であり、今回出なかったというだけ。**

  ### ★ 報告の誤りを2件訂正した

  1. **「箇条書き（`- `）もマークダウン記号を露出させることなくレンダリング」は誤り。**
     `40_trends_done.png` を見ると **`- ` は記号のまま表示されている。**
     `src/components/common.jsx` の `Markdown` を読んだところ、対応しているのは
     **見出し `#`・太字 `**`・インラインコード `` ` `` の3つだけで、リストは実装されていない。**
     つまり**仕様どおりで欠陥ではない**が、報告の記述は実物と食い違う。
     `docs/DEVICE_QA.md` の該当3箇所を実態に合わせて訂正した。
     **箇条書きに対応するかどうかは未決（未発注）。**
  2. **「全スクリーンショット（32枚）」は誤りで、実際は56枚（14MB）。**

  ### スクショは9枚だけコミットした

  `origin` が公開リポジトリのため、判定の根拠になる9枚だけを名指しで `git add` した
  （H-1 の4枚 / H-2 の2枚 / H-3 の3枚）。**残り47枚はローカルに残るが未追跡。**
  撮影対象はすべてアプリ内の画面で、ホーム画面や他プロジェクトの写り込みは無かった
  （前回 `00_lockscreen.png` で起きた問題は再発していない。発注文の指示が効いた）。

  ### 端末の後始末

  - **IME は現在 Simeji（端末既定）**で、放置された状態ではない。
    QA中に Google 日本語入力へ切り替えていたが、戻っている。
  - 今回のスクショは毎回 `/sdcard/sc.png` を `rm` しており、ゴミを残していない。
    **ただし 2026-08-23 の QA で置かれた `/sdcard/D2_*.png` 等が残ったままである**
    （端末への書き込みになるため Claude Code は消していない）。**未対応。**

- **2026-09-02 / Claude Code（追記49・公開状況の実測。進行役の過去の記述を訂正）**
  ユーザーから「ブラウザ版は https://evjou.pages.dev/ にある」と指摘を受けた。
  **追記33 に書いた「アプリ本体（`dist/`）はどこにも配信していない」は誤りだった。**
  リポジトリに配信設定（`wrangler.toml` / `_redirects` / `.github/` 等）が無いため
  そう判断したが、**Cloudflare Pages への配信はリポジトリ外で行われている。**
  **教訓: 「リポジトリに無い＝存在しない」ではない。外部環境はユーザーに聞くこと。**

  **実測（2026-09-02）:**

  | 対象 | 結果 |
  |---|---|
  | `https://evjou.pages.dev/` | **アプリ本体。バンドルは `index-aRV3PwtJ.js`** |
  | そのバンドルの中身 | `通常2時間以内に削除されます` / `キーと項目の対応` / `簡体字を使わないでください` / `直近7件` すべて**あり**。旧文言 `サーバー上にデータは保存されません` は**なし** |
  | `/landing/`・`/lp/` | **LPではない。**アプリの `index.html` が返る（SPAフォールバック） |
  | `/EvJou-release.apk` | **APKではない。**`Content-Type: text/html` が返る（同上） |

  **結論: ブラウザ版は最新（`cd56723` のビルド＝以後のコミットは文書のみなので現在の HEAD と同一）。
  LP はどこにも公開されていない。APK も配信されていない。**
  古いのは**リポジトリ内の `landing/EvJou-release.apk`（3,421,182 bytes / `versionCode=1`）だけ**で、
  これは誰にも配られていない。**追記33 ④の「公開すると嘘のビルドを配ることになる」は、
  実際にはまだ配っていないので、危険は顕在化していない。ただし LP を公開する時に顕在化する。**

  ### ユーザー決定（2026-09-02）

  - **LP は AI 連携を前面に出す。** 「オフライン基盤だがAI連携できる」がウリ。
  - 想定読者は **毎日（特に朝）ジャーナルを付けたい人** と **組織運営に携わる人**。
  - ブラウザ版も宣伝する。

  ### ★ 進行役が指摘した論点（未決・ユーザー判断待ち）

  1. **LP の置き場所が無い。** `evjou.pages.dev/` はアプリが占めている。
  2. **AI連携を前面に出すと、可用性の問題が顕在化する。**
     AI はユーザーの自宅PCのワーカー＋Ollama で動き、**自動起動も自動再起動も未設定**
     （`docs/operations.md` §5-3）。**訪問者が試すと高確率で「AIサーバーがメンテナンス中です」になる。**
  3. **「組織運営」の訴求は、不採用一覧と衝突しうる。**
     `docs/user-context.md` は**企業向け展開（認証・マルチユーザー・権限・サーバDB）を恒久的に不採用**と
     している。データは端末内のみで共有機能は無い。チーム機能があるかのように書くと
     **事故台帳4件目（LPと実態の乖離）の再発**になる。

- **2026-09-02 / Claude Code（追記50・ワーカー自動起動の検討と、Codex の実機操作許可）**

  ### ① ワーカーの自動起動（**実装していない。方針の検討のみ**）

  ユーザーの「別プロジェクトでタスクスケジューラに Ollama の起動を登録している」という前提を
  実測で確認したところ、**前提が違っていた。**

  | 対象 | 実測（2026-09-02） |
  |---|---|
  | Ollama の起動元 | **スタートアップフォルダの `Ollama.lnk`**（ログオン時）。PID 3876 / 2026-08-30 06:19 から稼働。**タスクスケジューラでもサービスでもない** |
  | タスクスケジューラの該当タスク | **「JournaLock 質問生成」のみ。毎日 07:30 に1回。** Ollama を起動する側ではなく**使う**側（`JOURNALOCK_LLM_MODEL=gemma4-8k` / `JOURNALOCK_EMBED_MODEL=bge-m3`）。直近 09/02 07:30:01 に結果 0 で正常終了 |

  **したがって「Ollama の二重起動」の心配は無い。** 干渉の実体は**GPUの取り合いだけ**で、
  空きVRAM 約5.4GB に対し qwen2.5:7b(4.4GB) と gemma4-8k(5.7GB) は同時常駐できず、
  入れ替えに16〜20秒かかる（追記40・追記41の実測）。
  **ぶつかるのは 07:30 前後に EvJou から AI を叩いたときだけで、症状は遅延。壊れない。**
  時間帯で止め合う調整は複雑さに見合わないと判断した。

  **★ 本当の論点は「自動起動するか」ではなく「常時稼働させるか」。**
  LP で AI を訴求する以上、可用性は要る。一方で常時稼働は
  **見知らぬ人の日記本文が常時この PC に流れる**ことを意味する。
  `DAILY_LIMIT` は**ユーザー単位**（`proxy/index.js` の `users/{uid}` を実測）なので、
  人数が増えれば総量はその分増える。

  **進行役の提案（未実施・ユーザー判断待ち）**:
  ① タスクスケジューラに「ログオン時に起動＋失敗時に再起動」を登録するが、**既定は無効**にしておく
  ② 公開直後は無効のまま（LPの「AI連携はテスト運用中」と整合する）
  ③ 反応を見てから有効化を判断する
  方式はスタートアップフォルダではなく**タスクスケジューラ**を推す（自動再起動と起動遅延が付けられるため）。
  **登録スクリプトは未起草。まだ何も登録していない。**

  ### ② Codex の実機操作を許可した（正典 §1 に「例外3」を追加）

  ユーザー決定（2026-09-02）により、**LP素材の撮影に限り、Codex が Mi 10 Pro（`a5f1d85`）を
  操作してスクリーンショットを撮ることを許可した。** 目的は Codex が自分で画面を見て
  自律的に構成を決められるようにすること。
  **正典 §1 の Codex の節に「例外3」として明記した**（対象端末・許可する操作・
  許可しない操作・実データの扱いを表で限定）。
  **これをやらないと Codex は正典との矛盾で正しく停止する**（正典 §1 冒頭の但し書き）。
  `install` / `uninstall` / `pm clear` / ワーカー操作は**引き続き禁止**で、
  APK の導入は Claude Code が事前に行う。

  ### ★ 未解決（着手前に片付けること）

  1. **Mi 10 Pro が adb に接続されていない**（`adb devices` に `1d05e7bc` のみ）。
     **接続してもらわないと APK も入れられず、Codex も撮影できない。**
  2. **Mi 10 Pro にはユーザー本人の実データが入っている。**
     撮った画像は公開LPに載るので、**日記本文や個人名が写ったものを素材にしてはいけない。**
     例外3の条文にこの制約を書いた。
  3. Mi 10 Pro のアプリは **`versionCode=1` / `1.0`（2026-07-30）のまま**。
     撮影の前に最新を `install -r` する必要がある（**Claude Code が実施する。未実施**）。

- **2026-09-02 / Claude Code（追記51・例外3の対象端末を OPPO へ変更）**
  Mi 10 Pro が使えないというユーザーの申告により、**正典 §1 の例外3の対象端末を
  `a5f1d85`（Mi 10 Pro）→ `1d05e7bc`（OPPO Reno A）へ変更した。**
  併せて「触ってはいけない端末」も入れ替えた（Mi 10 Pro が**実データ機**であることを明記）。
  - **これにより追記50 の未解決3件のうち2件が消えた。**
    OPPO は接続済み（`adb devices` で確認）で、アプリも **`versionCode=3` /
    `lastUpdateTime=2026-09-01 22:20:26`（追記46 で導入した最新）**が入っているため、
    **Claude Code による事前の APK 導入は不要。**
  - **実データの懸念も下がった。** OPPO は互換確認機で、入っているのはQAで作ったテストデータのみ。
    ただし**公開LPに載る以上、個人的な記述や固有名詞が写ったものを素材にしない**制約は残している。
  - **Mi 10 Pro は引き続き `versionCode=1` / `1.0`（2026-07-30）のままで、
    7/30 以降の修正が1件も届いていない。未着手。**

- **2026-09-02 / Claude Code（追記53・追記52のLP作業を検証。HTML/CSSは合格、素材は作り直しが要る）**

  ### 機械的な検査（すべて通過）

  | 検査 | 結果 |
  |---|---|
  | 変更範囲 | `landing/` 配下のみ（`index.html` / `styles.css` / 画像4枚）。`src/` `dist/` `proxy/` `android/` は無傷 |
  | CTA | 3箇所とも `https://evjou.pages.dev/` ＋ `target="_blank" rel="noopener"` |
  | `download` 属性 | **0件** |
  | APK 参照 | **0件**。`landing/EvJou-release.apk` は削除も変更もされていない |
  | 禁止語（チーム／共有／複数人／通知／リマインダー／検索機能） | **すべて0件** |
  | 退避 | `landing/assets/_old/` に旧4枚が保存されている |
  | AI可用性の注記 | 2箇所に掲載を確認 |
  | Mi 10 Pro | **無変更**（`versionCode=1` / `2026-07-30 22:52:46`）。作業中に接続されたが触られていない |

  ### 表示（合格）

  主幹の `landing/` をローカル配信して実測。**デスクトップ・モバイルとも破綻なく組めている。**
  ページ自体の横スクロールは発生しない（`docOverflowX: false`）。
  **★ 検証時の落とし穴**: `.claude/launch.json` の `landing` は**セッションの作業ディレクトリ基準**なので、
  入れ子ワークツリーから起動すると**古いLPが配信される**。主幹から起動し直して確認した。
  さらにブラウザが旧CSS・旧画像をキャッシュしていて別物が表示されたため、
  **別ホスト名（`127.0.0.1`）で開き直して確定させた。**

  ### ★★ 素材4枚のうち3枚が、公開LPに使えない

  | 画像 | 問題 |
  |---|---|
  | `shot_record.png`（LPの中心） | **ダンプ本文が英語**（`Good weather today. Friendly neighbor greeted me...`）。加えて主役の「✨ AIで整理する」が**無効状態（淡色）**で写っている（二度押しガードが効いた状態） |
  | `shot_ai_chat.png` | **質問が英語**（`Give me 3 tips for habit building.`）。入力欄にフォーカス枠が出ている |
  | `shot_sequence.png`（ヒーローの主役）／`shot_todo.png` | 中身がQAのテストデータで、**全件が赤い「期限切れ」**（`0/5 完了`・`⚠️ 期限切れ 5件`）。項目も「大目標を設定する」等の事務的なもの。**放置されたアプリに見える** |

  **原因は進行役の発注ミス。** 正典 §6 のとおり **adb から日本語を入力する手段は ADBKeyBoard IME だけ**だが、
  **実測すると OPPO には未導入**（`pm list packages` に無し。Mi 10 Pro にはある）。
  発注文にこの前提を書かなかったため、Codex は英語で打つしかなかった（正典 §3）。
  **これは事故台帳4件目（LP素材が空状態）と同種で、"素材が実態より悪く見える" 側の失敗である。**

  ### 報告の不正確 2件

  1. 「デスクトップ・モバイルとも横方向のはみ出しなし」→ **ページの横スクロールは無いが**、
     モバイル幅で `body.scrollWidth = 392 > 375` となり6要素がはみ出している。
     傾けた端末モックが画面外へ抜ける演出なので**意図的と推定**するが、「はみ出しなし」は不正確。
  2. 「画像7参照、欠落0」→ **ユニークな参照は5**（`app_icon` ＋ スクショ4枚）。

  ### 構成が大きく変わった（ユーザー判断が要る）

  ```
  旧: hero / how-it-works / killer-features / features / why / gallery / privacy / download （8節・画像8枚）
  新: top / flow / ai / local / try                                                        （5節・画像4枚）
  ```

  `shot_dump` / `shot_routine` / `shot_schedule` / `shot_settings` は**未使用**になった（削除はされていない）。
  発注の⑤で構成の判断を委ねたので範囲内だが、**LPの情報量が大きく減っている。残すかは要判断。**

  ### ★ 副次的な発見

  差し替え前の `shot_sequence.png`（いま `_old/` にある）には
  **「田中さんへ見積り送信」という人名を含む実データ**が写っていた。
  今回の差し替えで結果的に解消したが、**LPが未公開だったため実害は出ていない。**
  `_old/` に残っているので、**公開素材として再利用しないこと。**

- **2026-09-02 / Claude Code（追記54・LP素材の作り直しをA案で進める準備）**

  ### ユーザー決定

  1. **素材はブラウザ版から撮る（A案）。** 実機は使わない。
  2. **Mi 10 Pro（`a5f1d85`）は「テスト端末」である。** テスト用の日記を自由に作ってよい。
     **正典 §6 の端末表にあった「ユーザー本人の実データが入っている」は訂正した**
     （削除・アンインストール・データ消去に確認が要る点は据え置き。
     §8 の 2026-07-30 の消失事故は史実として残す）。
  3. LPの**改行位置と分かりやすさ**をもう一段上げる。

  ### `docs/demo-data.json` を用意した（`docs/TODO.md` 課題2 の解消）

  LP素材・ストア用スクショのための**デモデータ**。アプリの **設定 → インポート** で読み込める形式。

  ```
  日記 14日分（2026-08-20〜09-02）／ ToDo 8件（うち完了2・明日2・期限なし1）
  ルーチン 4件 ＋ 直近14日のチェック（達成率が8割前後に見えるようにしてある）
  目標3段 ／ ベーススケジュール1件 ／ 当日の生成済みスケジュール
  実在の人名・組織名は含めない
  ```

  **★ 進行役が実際に読み込んで確認済み。** `localhost:5174`（保存領域が空の別オリジン）で
  アプリ本来のインポート経路から取り込み、幅375pxで表示を実測した。
  `今日の稼働シーケンス（2/5 完了）`・目標3段・自然な日本語の記録が並び、
  **「全件が赤い期限切れ」だった追記53 の問題が解消することを確認した。**

  ### 例外3（Codex の実機操作）は使われないまま残る

  A案の決定により**LP素材の撮影に実機は使わない**ので、例外3は今回発動しない。
  条文は残すが、使う前にユーザー確認を取るよう本文に書き足した。
  **併せて記録**: `adb` から日本語を打てるのは ADBKeyBoard が入った端末だけで、
  **OPPO には未導入・Mi 10 Pro には導入済み**（2026-09-02 実測）。
  追記53 の「素材が英語になった」原因はこれである。

- **2026-09-03 / Claude Code（追記56・Codex の停止を切り分け。AIは動いていた）**
  Codex は「AI送信後の画面確認が3回タイムアウトしたので、指示どおり4回目は試さず停止」と報告した。
  **停止の判断そのものは正しい**（正典 §4-6）。ただし**原因の読みは誤り。**

  **実測: AIは正常に応答していた。** ワーカーのログに Codex の要求が残っている。

  ```
  処理開始: ReqID=EerIHfPOvhzccwftdFJ0, User=SecM604IH7bMupu2GSMZs255jJ92
  ✅ 処理完了: ReqID=EerIHfPOvhzccwftdFJ0 (本日 1/50 回)
  ```

  uid が新しいのは、Codex が別オリジン（`127.0.0.1:5173`）で開いたため匿名認証が新規になったから。
  **ワーカー（PID 37760）も Ollama も稼働中のまま。**

  **詰まったのはスクリーンショット。** 進行役も同じLP検証中に
  `Screenshot timed out after 5s: the page did not finish rendering in time` を踏んでいる（追記53）。
  **回避策**: ① スクショは1回失敗しても、そのまま撮り直すと通ることが多い
  ② 完了判定はスクショではなく `document.body.innerText` の読み取りで行う
  （AIの返答は DOM に入るので、テキストで確認する方が確実）。
  **この2点を発注文に書かなかったのは進行役の不足。**

  ### LPの構成についてのユーザー委任と、進行役の判断

  ユーザーは「LPを5節の簡潔版のままにするか、機能紹介を戻すか」を**進行役の判断に委ねた**（2026-09-03）。
  **判断: 5節の構成は維持し、機能の広がりが分かる一角だけを戻す。**
  理由は、簡潔な流れの方が読みやすく訴求も明確である一方、
  **画像が4枚では「日記アプリ止まり」に見え、予定・ルーチン・設定まで揃っている実態が伝わらない**ため。
  デモデータで中身のある画面が撮れるようになったので、戻すコストは低い。

- **2026-09-03 / Claude Code（追記58・追記57のLPを検証。文章と構成は合格、画像は作り直しが要る）**

  ### 合格した点

  - 変更は `landing/` 配下のみ（`index.html` 36/5・`styles.css` 127/2）。**今回は増分的な修正で、
    前回のような全面書き換えではない。**
  - 5節の構成を維持したまま、予定・ルーチン・設定が分かる小ギャラリーが入った（進行役の判断どおり）。
  - 差し替えた6枚とも**日本語で、中身のある画面**になった。`docs/demo-data.json` が効いている。
    ToDo画面は期限切れ・今日・期限未設定・完了済みの区分が見え、機能がよく伝わる。
  - 旧素材は `landing/assets/_old/` へ退避済み。

  ### ★★ 画像に技術的な欠陥がある（6枚とも作り直しが必要）

  ```
  新素材: JPEG image data, JFIF standard 1.01, 360x780   ← 拡張子は .png だが中身は JPEG
  旧素材: PNG image data, 1080 x 2340
  ```

  1. **拡張子と中身が食い違う。** Cloudflare Pages は拡張子で `Content-Type: image/png` を返すため
     不整合になる。UIのスクショは平坦な色と文字なので、**JPEG圧縮はにじむ**。PNGで保存すべき。
  2. **解像度が 360×780 しかない。** 旧素材の 1080×2340 に対し一辺で1/3。
     いまどきの端末は2〜3倍密度なので、**端末モックの中で明確にぼやける。**
     ブラウザ枠は `devicePixelRatio = 2` で描けている（進行役が実測）ので、
     **2倍の実ピクセルで保存すれば解決する。**

  ### ★★ Codex が AI画面を撮れなかった本当の原因が判明した

  追記56 で「スクショのタイムアウト」と書いたが、**それは結果で、原因ではなかった。**
  進行役が同じ手順を再現したところ、画面にこう出た。

  ```
  EvJou AIの利用には同意が必要です。もう一度実行すると確認画面が表示されます。
  ```

  **新しいオリジンで開くと初回同意が必要で、その `window.confirm`（ネイティブダイアログ）が
  自動操作を止める。** Codex は `127.0.0.1:5173` という新しいオリジンで開いたため、これを踏んだ。
  `innerText` もスクショも固まったのはダイアログが開いていたからで、**AIもワーカーも正常だった。**

  **回避策（実証済み）**: AIを使う前に IndexedDB の `ai-config` に `proxyConsent: true` を入れておく。
  実機QAで使ったのと同じ手（追記32）。

  **★ 進行役が実際に AI応答まで取得して確認した。** 同意を入れてから日本語で質問したところ、
  `処理開始 → ✅ 処理完了: ReqID=PUzRy9BRFNefsz5wniTP` が記録され、
  **自然な日本語の返答が Markdown の太字つきで描画された。LP素材として使える品質。**
  現在 AI節は設定画面で代用されているが、**本来の AI応答画面に差し替えるべき。**

  ### デモデータを撮影日に合わせられるようにした

  `docs/demo-data.json` は日付が固定なので、翌日に撮ると ToDo が「期限切れ」に見える
  （実際 09-02 生成のデータを 09-03 に撮った結果、期限切れ3件が写っている）。
  **`docs/gen-demo-data.mjs` を追加**し、`node docs/gen-demo-data.mjs` で当日基準に作り直せるようにした
  （`node docs/gen-demo-data.mjs 2026-09-10` で日付指定も可）。`docs/TODO.md` にも書いた。

- **2026-09-04 / Claude Code（追記60・追記59のLPを検証。形式は直った／レイアウトが違う）**

  ### 直った点（進行役が実測）

  | 項目 | 結果 |
  |---|---|
  | 画像の形式 | **7枚とも本物の PNG**（`\x89PNG` を確認）。拡張子との不一致は解消 |
  | 解像度 | **7枚とも 750×1624**（前回は 360×780 の JPEG）。ファイルサイズ 149〜353KB |
  | 変更範囲 | `landing/` 配下のみ |
  | AI応答画面 | **追加され、内容が良い。** デモデータの実際のToDo（`提案資料の骨子をつくる（〜9/4）`／`詰まっている件の次の一手を決める（〜9/4）`）を参照した日本語の返答で、**「AIが自分の記録を分かっている」ことが伝わる** |
  | 日本語の折り返し | **改善を確認。** 見出しが `頭が散らかった / 朝ほど、 / まず、全部書く。` と意味の切れ目で折れる（前回は `朝ほ/ど、`） |
  | モバイル幅 | ページ自体の横スクロールなし |
  | デモデータ | 撮影日に合わせて作り直されており、`今日の稼働シーケンス（2/5 完了）`。前回の「期限切ればかり」は解消 |

  ### ★★ ただし7枚とも「モバイル表示」ではない（要修正）

  **750px という数字は合っているが、出どころが違う。**
  **ビューポート幅を750pxにして撮っており、375pxのモバイル表示を2倍の画素で撮ったものではない。**

  根拠（画像を開いて確認）:
  - **タブが5つとも1行に収まっている。** 375px では「ルー／チン」と2行に折り返す
  - 本文が横に広く、`maxWidth 680px` のコンテナが余った幅に対して中央寄せになっている
  - 画面の下半分が空白（縦1624pxに対し内容が足りない）

  **影響は小さくない。** LPは `object-fit: cover; object-position: 50% 0%` で
  上端から切り出して端末モックへ嵌めているため、**横に広いレイアウトが細い枠に押し込まれ、
  ヒーローのモック内の文字が読めないほど小さくなっている。**
  下半分の空白自体は cover で切られるので実害はない（症状であって原因ではない）。

  **正しい撮り方**: **レイアウトは375px、画素だけ2倍**。
  **判定は簡単で、タブが「ルー／チン」と2行に折り返していれば正しい。**
  ビューポートを広げて750pxにしてはいけない。

- **2026-09-05 / Claude Code（追記62・LP素材の最終検証。合格）**
  追記61 の撮り直しを検証した。**3回かかったが、これで LP は公開できる状態になった。**

  | 項目 | 結果 |
  |---|---|
  | 形式・解像度 | **7枚とも本物のPNG・750×1624**（進行役がヘッダを読んで確認） |
  | **レイアウト** | **モバイル表示で撮れている。** `shot_sequence.png` と `shot_ai_chat.png` を開いて、**タブが「ルー／チン」と2行に折り返している**ことを確認した |
  | 内容 | 日付は当日（2026/09/05）。`今日の稼働シーケンス（2/5 完了）`で完了済み2件が緑。**期限切ればかりだった問題も、英語混入も解消** |
  | AI応答画面 | デモデータの実タスク（提案資料の骨子／詰まっている件の次の一手／夕方の振り返り）を参照した日本語の返答 |
  | LP上の見え方 | **端末モックの中身が読めるようになった**（前回は判読不能）。8枚とも欠落なし、750px素材を327〜381pxで表示＝**約2倍密度** |
  | 横方向のはみ出し | なし |
  | 変更範囲 | `landing/assets/` の7枚のみ。`index.html` / `styles.css` は差分ゼロ（指示どおり） |

  **★ 残る軽微な点（未対応・公開の判断はユーザー）**:
  `shot_sequence.png` と `shot_ai_chat.png` に、プログラム操作で押したボタンの
  **黒いフォーカス枠**が残っている。宣材としてはわずかに気になるが、
  LP上では約330px幅に縮むため細い線として見える程度。**次に素材を触るときに直せばよい。**

  **★ 進行役の発注ミス（記録）**: 「書き込んでよいのは `landing/assets/` 配下のみ」と書きながら
  `node docs/gen-demo-data.mjs`（`docs/` へ書き込む）を実行させる矛盾した指示を出した。
  Codex は `EPERM` を踏み、一時領域で生成して回避した。**判断は妥当。**
  `docs/demo-data.json` は内容が HEAD と同一（差分は改行コードのみ）だったので進行役が戻した。
