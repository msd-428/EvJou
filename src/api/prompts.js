...
# 既存のフォーム内容（変更しない。重複を避けるための参考として読むだけ）
${current}

# 処理タスク
1. ダンプテキストの内容だけを上記${fields.length}個の項目に振り分ける。
2. 既存のフォーム内容に既に書かれていることは返さず、新しく足すぶんだけを返す。足すものが無い項目は空文字にする。
3. 原文にない情報を勝手に追加しない。
4. タスクが羅列されている場合はMECEに整理し、実行する順序（朝→夜の物理動線）に沿って並べ替える。精神論ではなく動作で書く。
